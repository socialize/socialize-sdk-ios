 ...
 */
- (SocializeFBRequest*)openUrl:(NSString *)url
  ...
  [_request release];
  _request = [[SocializeFBRequest getRequestWithParams:params
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    if (tryFBAppAuth) {
      ...
      NSString *fbAppUrl = [SocializeFBRequest serializeURL:urlPrefix params:params];
    ...
    if (trySafariAuth && !didOpenOtherApp) {
      ...
      NSString *fbAppUrl = [SocializeFBRequest serializeURL:loginDialogURL params:params];
      didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }
  }
  ...
  if (!didOpenOtherApp) {
    [_loginDialog release];
    _loginDialog = [[SocializeFBLoginDialog alloc] initWithURL:loginDialogURL
                                          loginParams:params
                                             delegate:self];
    [_loginDialog show];
  }
...
- (void)dialog:(NSString *)action
  ...
  if (action == kLogin) {
    ...
    _fbDialog = [[SocializeFBLoginDialog alloc] initWithURL:dialogURL loginParams:params delegate:self];
  } else {
    ...
    _fbDialog = [[SocializeFBDialog alloc] initWithURL:dialogURL params:params delegate:delegate];
  }
